...
import QuickServiceCard from '@/components/ui/home-ios/QuickServiceCard.vue'
import { Bell, ArrowUp, Trash2 } from 'lucide-vue-next'
...
// Respect prefers-reduced-motion for scroll behavior in carousels
const { prefersReduced } = usePrefersReducedMotion()
const scrollStyle = computed(() => (prefersReduced.value ? {} : { scrollBehavior: 'smooth' }))
...
<template>
  <div class="home-ios min-h-full bg-linear-to-br from-stone-50 via-white to-emerald-50  overflow-y-auto">
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none z-0" aria-hidden="true">
        <div aria-hidden="true" class="absolute top-24 left-8 w-56 h-56 bg-purple-100 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-float" />
        <div aria-hidden="true" class="absolute top-40 right-16 w-80 h-80 bg-purple-200 rounded-full mix-blend-multiply filter blur-3xl opacity-16 animate-float" style="animation-delay: -2s;" />
        <div aria-hidden="true" class="absolute bottom-24 left-1/3 w-64 h-64 bg-purple-50 rounded-full mix-blend-multiply filter blur-3xl opacity-18 animate-float" style="animation-delay: -4s;" />
      </div>

    <!-- Main Content -->
    <main role="main" aria-label="Panel principal" class="relative z-10 pt-6 pb-12 px-6 max-w-7xl mx-auto space-y-10">
      <h1 class="sr-only">Panel Principal</h1>
      
      <!-- Welcome Banner -->
      <header class="hero relative z-10">
      <WelcomeBanner 
        :nombre="nombreCompleto || 'Usuario'"
        :saludo="saludo"
        to-nueva-cita="/citas"
        to-ver-historial="/controles"
      />
      </header>

      <!-- Quick Services (fila centrada estilo iOS) -->
      <section aria-label="Accesos rápidos" class="py-6">
        <Motion :initial="{ opacity: 0, y: 8 }" :animate="{ opacity: 1, y: 0 }" :transition="{ duration: 0.45, delay: 0.15 }">
        ...
        >
          <div class="flex items-center justify-between mb-6">
            ...
            <RouterLink
              ...
              to="/recursos"
              class="text-purple-600 hover:text-purple-700 font-medium text-sm flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/40 focus-visible:ring-offset-2 focus-visible:ring-offset-white rounded-md"
            >
              Ver todo
