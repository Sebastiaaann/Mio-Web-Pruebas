...
describe('tiendaUsuario', () => {
  ...
  describe('Login', () => {
    it('debería iniciar sesión exitosamente', async () => {
      ...
      expect(authService.guardarSesion).toHaveBeenCalledWith('mock-token', mockUser)
    })
    ...
    it('debería manejar excepciones durante login', async () => {
      ...
      authService.iniciarSesion.mockRejectedValue(new Error('Error de red'))
    ...
    it('debería establecer cargando en true durante login', async () => {
      ...
      authService.iniciarSesion.mockImplementation(() => 
        new Promise(resolve => setTimeout(() => resolve({ success: true, token: 'token', user: {} }), 100))
      )
  ...
  describe('Restaurar sesión', () => {
    it('debería restaurar sesión desde localStorage', () => {
      ...
      const mockSession = {
        token: 'stored-token',
        user: { uid: '123', patient_id: 456 }
      }
      
      authService.restaurarSesion.mockReturnValue(mockSession)
