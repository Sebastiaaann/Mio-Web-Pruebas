 ...
 * Útil para eventos @input en formularios
 * @param {string} rut - RUT ingresado por el usuario
 * @returns {string} RUT formateado automáticamente
 */
export function formatRutOnInput(rut) {
  if (!rut) return '';
 ...
 */
export function obtenerNumeroRut(rut) {
  if (!rut) return '';
  const rutLimpio = limpiarRut(rut);
 ...
 */
export function obtenerDvRut(rut) {
  if (!rut) return '';
  const rutLimpio = limpiarRut(rut);
 ...
 * @param {string} rut - RUT a validar y formatear
 * @returns {{valido: boolean, rutFormateado: string, error?: string}}
 */
export function validarYFormatearRut(rut) {
  if (!rut) {
    return {
      valido: false,
      rutFormateado: '',
